...
# write new function that gets cif file values waithout using as_number OR rewrite get_fract_vals function to call get_std_dev within it and store the fract and std_dev in separate variables
class CifExtractor:
    ...
    def get_atomic_site_symbol_mat(self):
        atomic_symbol_mat = CifExtractor.get_atomic_site_type_symbol('_atom_site_type_symbol', self.block)
        return atomic_symbol_mat
    ...
    # extract atomic site type symbol from CIF file into matrix of strings
    def get_atomic_site_type_symbol(atomic_symbol, block):
        atomic_site_type_symbol = [cif.as_string(i) for i in (block.find_loop(atomic_symbol))]
        return atomic_site_type_symbol
